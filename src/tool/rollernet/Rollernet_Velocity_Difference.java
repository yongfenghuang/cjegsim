...
public class Rollernet_Velocity_Difference {
		int loglevel = 1000;
		...
		public static void main(String[] args) {
			String outfile="rollernet/velocity_diff.txt";
			boolean debug=false;
			try {
				...
				Rollernet_Velocity_Difference calculator = new Rollernet_Velocity_Difference();
			...
			for (int i=0;i<activenodes.size();i++)
				for(int j=i+1;j<activenodes.size();j++){
				    ...
				    if (va!=0 || vb!=0){
				    	...
				    	Log.writeln("diff between "+source+" and "+destination+" is:"+df.format(diff), loglevel);
				    	average_diff=average_diff+diff;
				    	diffnumber++;
				    }
				}
			Log.writeln("total diff is:"+average_diff, loglevel);
			average_diff=average_diff/(double)diffnumber;
			Log.writeln("node pair is:"+diffnumber, loglevel);
			Log.writeln("average diff is:"+df.format(average_diff), loglevel);
			return average_diff;
		}
		
		public double getVelocity(int source,int destination) {
			double velocity=0;
			String sqlstr1 = "SELECT velocity from cjeg_velocity_200 WHERE source ="
					+ source
					+ " AND destination ="
					+ destination;
			ResultSet rs = null;
